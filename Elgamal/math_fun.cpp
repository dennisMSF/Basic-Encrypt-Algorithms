...
ZZ gen_prime2p_1(int a){
    ZZ result;
    while(true)
    {
        ZZ prime=ZZ(GenPrime_ZZ(a));
        result=prime*2+1;
        if(ProbPrime(result))return result;

    }
}
ZZ gen_root(const ZZ& p){
    ZZ q=(p-1)>>1;
    ZZ e(conv<ZZ>(2));
    string e_bin=binario(e);
    string q_bin=binario(q);
    for(ZZ i(conv<ZZ>(2));i<p;++i)
      if(bynexpo(i,e_bin,p)!=1&&bynexpo(i,q_bin,p)!=1)return i;
}
ZZ gen_d(int a, const ZZ& p){
    ZZ t=ZZ(RandomLen_ZZ(a));
    t=modulo(t,p);
    if(t<2)
        return t+2;
    else if(t+2>p)
...
ZZ diffie(ZZ p,ZZ g,ZZ x){
    string bin_x=binario(x);
    ZZ pasar=bynexpo(g,bin_x,p);
    return pasar;
}
ZZ hellman(ZZ p,ZZ x,ZZ ext){
    string bin_x=binario(x);
    ZZ k_comun=bynexpo(ext,bin_x,p);
}
